(function(root){
  ...
  async function staticParts(entry){
    if(!entry)return[];const parts=entry.parts||entry.files||[];let bars=[];
    for(const part of parts){const path=typeof part==='string'?part:part.path;if(!path)continue;const response=await fetch(new URL(path,baseUrl()),{cache:'force-cache'});if(!response.ok)throw new Error(`static part ${path} unavailable`);const payload=await response.json();bars=mergeBars(bars,Array.isArray(payload)?payload:payload.rows||[])}
    return bars;
  }
  async function load(asset,start,end,remoteLoader){
    ...
    if(entry){try{bars=mergeBars(bars,await staticParts(entry));source='GitHub静态历史库';persisted=true}catch(error){if(!bars.length)throw error}}
    if(!covers(bars,start,end)&&typeof remoteLoader==='function'){
      const remoteStart=bars.length&&bars.at(-1).date>=start?nextDay(bars.at(-1).date):start;
      if(remoteStart<=end){const value=await remoteLoader(remoteStart,end);bars=mergeBars(bars,value.bars||[]);source=persisted?'静态历史 + 东方财富增量':'东方财富即时日线（已存本机）';persisted=false}
    }
